...
template <class dtype>
void medianfilter<dtype>::MedianUtil(dtype arr[], int l, int r,
                int k, dtype& a, dtype& b)
{
...
template <class dtype>
int medianfilter<dtype>::randomPartition(dtype arr[], int l,
                    int r)
{
    int n = r - l + 1;
    int pivot = rand() % n;
...
template <class dtype>
void medianfilter<dtype>::swap(dtype* a, dtype* b)
{
    dtype temp = *a;
    *a = *b;
    *b = temp;
